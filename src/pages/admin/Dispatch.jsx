...
const Dispatch = () => {
  ...
  const filteredRides = rideRequests.filter((ride) => {
    ...
    const driverName = ride.driver
      ...
      : ''
    const start = (ride.startAddress || '').toLowerCase()
    const end = (ride.endAddress || '').toLowerCase()
  ...
  const pendingCount = rideRequests.filter((r) => r.status === 'pending' || r.status === 'accepted' || r.status === 'in_progress').length
  const cancelledCount = rideRequests.filter((r) => r.status === 'cancelled').length

  const statCards = [
    {
      label: language === 'ar' ? 'إجمالي الطلبات' : 'Total rides',
      value: totalCount,
      icon: FiTruck,
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] gap-4">
        ...
        <p className="text-gray-500 dark:text-gray-400 font-medium">
          {language === 'ar' ? 'جاري تحميل الطلبات...' : 'Loading dispatch...'}
        </p>
      </div>
    )
  }
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-base text-gray-600 dark:text-gray-400 max-w-2xl">
            {language === 'ar'
              ? 'عرض وإدارة طلبات الركوب — تعيين السائقين ومتابعة الحالة.'
              : 'View and manage ride requests — assign drivers and track status.'}
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-1 flex-col gap-4 sm:flex-row sm:items-center">
            ...
            >
              ...
              <option value="in_progress">{t('in_progress', language) || 'In progress'}</option>
        ...
        {filteredRides.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 px-6">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white text-center">
              {t('noRideRequestsFound', language)}
            </h3>
            <p className="mt-2 text-center text-gray-500 dark:text-gray-400 max-w-md">
              {searchTerm || statusFilter !== 'all'
                ...
                : (language === 'ar' ? 'لا توجد طلبات ركوب حتى الآن.' : 'No ride requests yet.')}
            </p>
          </div>
        ) : viewMode === 'cards' ? (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex min-w-0 flex-1 gap-3">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-orange-400 to-orange-600 text-lg font-bold text-white">
                      {(ride.rider?.firstName?.[0] || '?').toUpperCase()}
                      {(ride.rider?.lastName?.[0] || '').toUpperCase()}
                    </div>
                  ...
                  >
                    {t(ride.status || 'pending', language) || ride.status || 'pending'}
                  </span>
                </div>
                ...
                <div className="mt-4 flex items-center justify-between border-t border-gray-100 dark:border-gray-700 pt-4">
                  ...
                  {!ride.driver && (
                    <button
                      type="button"
                      className="text-sm font-medium text-orange-600 dark:text-orange-400 hover:text-orange-800 dark:hover:text-orange-300"
                    >
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-orange-400 to-orange-600 text-white text-sm font-semibold">
                          {(ride.rider?.firstName?.[0] || '?').toUpperCase()}
                        </div>
