...
const ServiceCategories = () => {
  ...
  const withVehicleCount = categories.filter((c) => (c.vehicleCategories?.length || 0) > 0).length
  const statCards = [
    { label: language === 'ar' ? 'إجمالي الفئات' : 'Total categories', value: totalCount, icon: FiLayers, bgLight: 'bg-slate-50 dark:bg-slate-900/30', iconColor: 'text-slate-600 dark:text-slate-400', borderColor: 'border-slate-200 dark:border-slate-700' },
    ...
    { label: t('vehicleCategoriesCount', language), value: withVehicleCount, icon: FiLayers, bgLight: 'bg-orange-50 dark:bg-orange-900/20', iconColor: 'text-orange-600 dark:text-orange-400', borderColor: 'border-orange-200 dark:border-orange-800' },
  ]
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-lg overflow-hidden">
        {filteredCategories.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 px-6">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white text-center">{t('noServiceCategoriesFound', language)}</h3>
            <p className="mt-2 text-center text-gray-500 dark:text-gray-400 max-w-md">{searchTerm ? t('tryAdjustingYourSearch', language) : (language === 'ar' ? 'لا توجد فئات خدمات حتى الآن.' : 'No service categories yet.')}</p>
          </div>
        ) : viewMode === 'cards' ? (
              ...
              <div key={category.id} className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5 shadow-sm hover:shadow-md hover:border-orange-200 dark:hover:border-orange-800 transition-all duration-200">
                ...
                <div className="mt-3 flex items-center gap-2">
                  <span className="px-2 py-0.5 text-xs font-medium rounded bg-orange-50 dark:bg-orange-900/20 text-orange-800 dark:text-orange-200 border border-orange-200 dark:border-orange-700">
                    {category.vehicleCategories?.length || 0} {t('vehicleCategoriesCount', language)}
                  </span>
                </div>
                <div className="mt-4 flex items-center justify-end border-t border-gray-100 dark:border-gray-700 pt-4">
                  <ActionButtons onView={() => navigate(`/service-categories/${category.id}`)} onEdit={() => handleOpenModal(category)} onDelete={() => handleDelete(category.id)} showView={true} showEdit={true} showDelete={true} forceShowIcons={true} size="sm" />
                </div>
              </div>
            ))}
          </div>
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredCategories.map((category) => (
                  <tr key={category.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        {category.icon && <span className="text-xl" role="img" aria-hidden>{category.icon}</span>}
                        <div>
                          ...
                          {category.nameAr && category.name && <div className="text-xs text-gray-500 dark:text-gray-400">{language === 'ar' ? category.name : category.nameAr}</div>}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="px-2 py-0.5 text-xs font-medium rounded bg-orange-50 dark:bg-orange-900/20 text-orange-800 dark:text-orange-200 border border-orange-200 dark:border-orange-700">{category.slug || '-'}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="px-3 py-1 inline-flex text-xs font-semibold rounded-full border bg-orange-50 dark:bg-orange-900/20 text-orange-800 dark:text-orange-200 border-orange-200 dark:border-orange-700">{category.vehicleCategories?.length || 0}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <ActionButtons onView={() => navigate(`/service-categories/${category.id}`)} onEdit={() => handleOpenModal(category)} onDelete={() => handleDelete(category.id)} showView={true} showEdit={true} showDelete={true} forceShowIcons={true} />
                    </td>
                  </tr>
                ))}
              </tbody>
