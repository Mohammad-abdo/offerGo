...
const VehicleCategoriesPassenger = () => {
  ...
  const statCards = [
    { label: language === 'ar' ? 'إجمالي الفئات' : 'Total categories', value: totalCount, icon: FiUsers, bgLight: 'bg-slate-50 dark:bg-slate-900/30', iconColor: 'text-slate-600 dark:text-slate-400', borderColor: 'border-slate-200 dark:border-slate-700' },
        ...
        {filteredCategories.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 px-6">
            <div className="flex h-24 w-24 items-center justify-center rounded-full bg-orange-100 dark:bg-orange-900/30 mb-6">
              <FiUsers className="text-orange-500 dark:text-orange-400" size={48} />
            </div>
            <h3 className="text-xl font-bold text-gray-900 dark:text-white text-center">{t('noPassengerCategoriesFound', language)}</h3>
            <p className="mt-2 text-center text-gray-500 dark:text-gray-400 max-w-md">{searchTerm ? t('tryAdjustingYourSearch', language) : (language === 'ar' ? 'لا توجد فئات ركاب حتى الآن.' : 'No passenger categories yet.')}</p>
          </div>
        ) : viewMode === 'cards' ? (
            ...
            {filteredCategories.map((category) => (
              <div key={category.id} className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5 shadow-sm hover:shadow-md hover:border-orange-200 dark:hover:border-orange-800 transition-all duration-200">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <span className="px-2 py-0.5 text-xs font-medium rounded bg-orange-50 dark:bg-orange-900/20 text-orange-800 dark:text-orange-200 border border-orange-200 dark:border-orange-700">{category.capacity ?? '-'} {t('seats', language)}</span>
                  <span className="px-2 py-0.5 text-xs font-medium rounded bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300">{category.features?.length ?? 0} {t('features', language)}</span>
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredCategories.map((category) => (
                  <tr key={category.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">{category.features?.length ?? 0}</td>
